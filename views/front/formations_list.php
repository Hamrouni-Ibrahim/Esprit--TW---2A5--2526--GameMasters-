<?php 
$pageTitle = 'Formations - Game Master';
...
<section class="content-section">
    ...
    <div class="content-container">
        <h2 class="section-title">Liste des Formations</h2>
        ...
        <div style="max-width: 600px; margin: 30px auto; background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(232, 121, 249, 0.2); border-radius: 15px; padding: 25px;">
            <form method="GET" action="" style="display: flex; gap: 10px; align-items: center;">
                ...
                <?php if (isset($_GET['search']) && !empty($_GET['search'])): ?>
                    <a href="?controller=formation&action=list" style="padding: 12px 20px; background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2); border-radius: 10px; color: #fff; text-decoration: none; font-weight: 600;">
                        ✕ Effacer
                    </a>
                <?php endif; ?>
            </form>
            ...
            <div class="items-grid">
                <?php foreach($results as $row) { ?>
                    <div class="item-card">
                        ...
                        <a href="?controller=formation&action=detail&id=<?php echo $row['id']; ?>" class="btn btn-primary">Voir plus</a>
                    </div>
                <?php } ?>
            </div>
        <?php } else { ?>
            <p class="no-items">
                <?php if (isset($_GET['search']) && !empty($_GET['search'])): ?>
                    Aucune formation trouvée pour "<?php echo htmlspecialchars($_GET['search']); ?>".
                <?php else: ?>
